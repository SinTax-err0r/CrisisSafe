...
{archive_html}
    <div class="credibility-score" style="{score_style}">{display_score}</div>
    <div class="credibility-label">{display_label}</div>
</div>
""", unsafe_allow_html=True)
            ...
            with tab2:
                ...
                for i, pointer in enumerate(st.session_state.pointers, 1):
                    ...
                    <div class="pointer-item">
                        <div class="pointer-text">{pointer}</div>
                    </div>
                    """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if "related_articles" in st.session_state and st.session_state.related_articles:
        ...
        for i, article in enumerate(st.session_state.related_articles):
            body_html = article.get('highlighted_body', article['body'])
            ...
            <div class="news-card">
                ...
                <div class="news-card-source">SOURCE: {article['url'][:30]}...</div>
                <div class="news-card-snippet">{clean_body}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col2:
                if st.button("Verify This", key=f"sidebar_verify_{i}", use_container_width=True):
                    st.session_state.claim_input = article['url']
                    st.rerun()
    else:
